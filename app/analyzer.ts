...
type AgingLot={date:Date;amount:number;ref?:string;dueDate?:Date};
...
function effectiveDueDate(lot:AgingLot,creditDays:number){if(lot.dueDate&&lot.dueDate.getTime()>=lot.date.getTime())return new Date(lot.dueDate);const date=new Date(lot.date);date.setDate(date.getDate()+creditDays);return date}
...
function prepareLedgerAging(d:DataSet,supplier:boolean,closing:number,now:Date,creditDays=30){
  const debit=column(d,"debit"),credit=column(d,"credit"),bal=column(d,"balance"),amount=column(d,"amount"),date=column(d,"date"),invoiceDate=column(d,"invoiceDate"),dueDateColumn=column(d,"due"),ageDate=invoiceDate||date,ref=column(d,"ref"),sign=closing<0?-1:1,polarity=bal?ledgerPolarity(d,debit,credit,bal):(supplier?-1:1);let lots:AgingLot[]=[],previousBalance:number|null=null,fallbackDate:Date|null=null;
  const transactionRows=d.rows.filter(row=>!!parseDate(value(row,ageDate))||(!!bal&&String(value(row,bal)??"").trim()!==""));
  for(const row of transactionRows){const issueDate=parseDate(value(row,ageDate)),lotDate=issueDate||now,explicitDue=parseDate(value(row,dueDateColumn));fallbackDate=issueDate||fallbackDate;const rawBalance=value(row,bal),hasBalance=!!bal&&String(rawBalance??"").trim()!=="";let change=0;if(hasBalance){const current=number(rawBalance)*sign;change=previousBalance===null?polarity*(number(value(row,debit))-number(value(row,credit))):current-previousBalance;previousBalance=current}else if(debit||credit){const dr=number(value(row,debit)),cr=number(value(row,credit));change=polarity*(dr-cr)}else if(amount)change=number(value(row,amount));if(change>.001)lots.push({date:lotDate,amount:change,ref:String(value(row,ref)||"—"),dueDate:explicitDue&&explicitDue.getTime()>=lotDate.getTime()?explicitDue:undefined});else if(change<-.001)lots=consumeOldest(lots,-change)}
  lots=reconcileLots(lots,Math.abs(closing),fallbackDate||now).sort((a,b)=>a.date.getTime()-b.date.getTime());
  const explicitDueCount=lots.filter(lot=>lot.dueDate).length,basis=invoiceDate?`العمر محسوب من تاريخ إصدار كل فاتورة حتى آخر تاريخ في الكشف. ${explicitDueCount?`تم اعتماد تاريخ الاستحقاق الصريح في ${explicitDueCount} فاتورة،`:"عند عدم وجود تاريخ استحقاق صريح تُضاف مدة الائتمان المختارة،"} مع توزيع المردودات والسداد على أقدم فاتورة أولًا (FIFO).`:date?`العمر محسوب من التاريخ الموجود في سطر الفاتورة. ${explicitDueCount?"تاريخ الاستحقاق الصريح له الأولوية،":"تُضاف مدة الائتمان المختارة عند غياب تاريخ الاستحقاق،"} مع توزيع الخصومات على أقدم فاتورة أولًا (FIFO).`:"لم يظهر تاريخ فاتورة صالح؛ لا يمكن اعتماد توزيع الأعمار دون تاريخ الإصدار.";
  ...
  return{aging,rows,openCount:lots.length,basis,dueTotal,nonDueTotal,dueScenarios,explicitDueCount};
}
...
function ledger(ds: DataSet[], supplier=false,creditDays=30): AnalysisResult {
  ...
  const findings:AnalysisResult["findings"]=[
    ...
    {title:"أولوية تاريخ الاستحقاق",detail:prepared.explicitDueCount?`تم العثور على تاريخ استحقاق صريح وربطه بالفاتورة في ${prepared.explicitDueCount} بند؛ تم اعتماده بدل المدة الافتراضية. الفواتير الأخرى تستخدم مدة ${creditDays} يومًا.`:`لم يظهر تاريخ استحقاق صريح؛ تم احتساب تاريخ الاستحقاق من تاريخ الفاتورة + ${creditDays} يومًا.`,tone:"ok"},
