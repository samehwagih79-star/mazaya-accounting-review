...
export type AnalysisResult = {
  ...
  dueSchedule?: { asOf: string; includedThrough: string; cutoffInvoiceDate?: string; cutoffBalance?: string; laterDeductions?: string; nextDueDate?: string; nextDueAmount?: string; nextDueInvoicesAmount?: string; nextDueRef?: string; nextDueDays?: number; nextDueCreditDays?: number };
...
function remainingDays(asOf:Date,dueDate:Date){return Math.max(0,Math.ceil((dueDate.getTime()-asOf.getTime())/86400000))}
function dueTiming(invoiceDate:Date,asOf:Date,creditDays:number){const dueDate=effectiveDueDate({date:invoiceDate,amount:0},creditDays),difference=Math.floor((asOf.getTime()-dueDate.getTime())/86400000);return{dueDate,status:difference>=0?"مستحق":"غير مستحق",timing:difference>=0?`متأخر ${difference} يومًا`:`متبقي ${Math.abs(difference)} يومًا`}}
...
const aliases: Record<string, string[]> = {
  ...
  ref: ["المرجع", "رقم المرجع", "reference", "ref", "رقم القيد", "رقم المستند", "رقم السند", "رقم الفاتوره"],
...
function pdfOpenLots(review:PdfReview){
...
function pdfDueRows(review:PdfReview,asOf:Date,creditDays:number){return pdfOpenLots(review).map(lot=>{const age=Math.max(0,Math.floor((asOf.getTime()-lot.date.getTime())/86400000)),timing=dueTiming(lot.date,asOf,creditDays),range=agingRanges(creditDays).find(item=>age>=item.min&&age<=item.max);return[lot.ref||"—",pdfDateFormatter.format(lot.date),pdfDateFormatter.format(timing.dueDate),`تاريخ الفاتورة + ${creditDays} يومًا`,String(age),range?.label||`${age} يومًا`,timing.timing,timing.status,money(lot.amount)]})}

function preparePdfDue(review:PdfReview,supplier:boolean,asOf:Date,creditDays:number){
  const sign=review.derivedClosing<0?-1:1,rows=review.movements.map((movement,index)=>{const balance=Math.max(0,movement.balance*sign),change=index?balance-Math.max(0,review.movements[index-1].balance*sign):movement.isOpening?0:movement.increase-movement.decrease,text=accountingSearchText(`${movement.raw} ${movement.description}`),invoice=/فاتوره.*(?:مبيعات|بيع|مشتريات|شراء)|(?:مبيعات|مشتريات).*فاتوره|sales invoice|purchase invoice|supplier invoice/.test(text)&&change>.001,deduction=supplier?/مردود.*مشتريات|مرتجع|سند.*(?:صرف|يوميه)|حواله|سداد|دفع|تحويل|payment|paid|bank transfer/.test(text)&&change<-.001:/مردود.*مبيعات|مرتجع|سند.*قبض|تحصيل|قبض|تحويل|receipt|payment|paid/.test(text)&&change<-.001;return{date:movement.date,balance,change,index,isInvoice:invoice,isDeduction:deduction,ref:movement.description}}),calculate=(days:number)=>{const cutoff=rows.filter(row=>row.isInvoice&&isLotDue({date:row.date,amount:Math.max(0,row.change)},asOf,days)).at(-1);if(!cutoff)return{value:0,deductions:0,cutoff:undefined as typeof rows[number]|undefined};const deductions=rows.filter(row=>row.index>cutoff.index&&row.isDeduction).reduce((sum,row)=>sum-row.change,0);return{value:Math.max(0,cutoff.balance-deductions),deductions,cutoff}},scenarioDays=[...new Set([30,45,60,creditDays])].filter(days=>days>=30&&days<=60).sort((a,b)=>a-b),periodName=(days:number)=>days===30?"شهر":days===45?"شهر ونصف":days===60?"شهران":`${days} يومًا`,selected=calculate(creditDays),future=rows.filter(row=>row.isInvoice&&!isLotDue({date:row.date,amount:Math.max(0,row.change)},asOf,creditDays)).sort((a,b)=>a.date.getTime()-b.date.getTime())[0],total=Math.abs(review.closing);
  const futureDueDate=future?effectiveDueDate({date:future.date,amount:0},creditDays):undefined,futureCutoff=future?rows.filter(row=>row.isInvoice&&row.date.getTime()===future.date.getTime()).at(-1):undefined,futureDeductions=futureCutoff?rows.filter(row=>row.index>futureCutoff.index&&row.isDeduction).reduce((sum,row)=>sum-row.change,0):0,futureBalance=futureCutoff?Math.max(0,futureCutoff.balance-futureDeductions):0;
  const futureInvoicesAmount=futureCutoff&&future?rows.filter(row=>row.isInvoice&&row.date.getTime()===future.date.getTime()&&row.index<=futureCutoff.index).reduce((sum,row)=>sum+Math.max(0,row.change),0):0;
  return{dueTotal:selected.value,nonDueTotal:Math.max(0,total-selected.value),dueScenarios:{selectedDays:creditDays,items:scenarioDays.map(days=>({days,label:`المستحق بعد ${periodName(days)} (${days} يومًا)`,value:money(calculate(days).value),selected:days===creditDays}))},dueSchedule:{asOf:pdfDateFormatter.format(asOf),includedThrough:selected.cutoff?pdfDateFormatter.format(effectiveDueDate({date:selected.cutoff.date,amount:0},creditDays)):"لا توجد فاتورة مستحقة",...(selected.cutoff?{cutoffInvoiceDate:pdfDateFormatter.format(selected.cutoff.date),cutoffBalance:money(selected.cutoff.balance),laterDeductions:money(selected.deductions)}:{}),...(future&&futureDueDate?{nextDueDate:pdfDateFormatter.format(futureDueDate),nextDueAmount:money(futureBalance),nextDueInvoicesAmount:money(futureInvoicesAmount),nextDueRef:future.ref,nextDueDays:remainingDays(asOf,futureDueDate),nextDueCreditDays:creditDays}:{})}};
}
...
function pdfAnalysis(action:string,ds:DataSet[],creditDays=30):AnalysisResult{
  ...
  const reportDate=single?parseDate(ds[0].reportAsOf):null,asOf=reportDate?new Date(reportDate):end;asOf.setHours(0,0,0,0);const pdfDue=single&&(supplier||customer)&&closingUsable?preparePdfDue(single,supplier,asOf,creditDays):undefined,dueValue=pdfDue?money(pdfDue.dueTotal):closingValue,conclusion=single&&(supplier||customer)?{label:dueLabel,value:dueValue,detail:pdfDue?`تم أخذ الرصيد الجاري ${pdfDue.dueSchedule.cutoffBalance||money(0)} عند آخر فاتورة حل استحقاقها، ثم خصم ${pdfDue.dueSchedule.laterDeductions||money(0)} من ${supplier?"مردودات المشتريات وسندات الصرف والسداد":"مردودات المبيعات وسندات القبض والتحصيل"} اللاحقة. الفواتير الأحدث غير المستحقة لم تدخل في المبلغ.`:"لم تظهر قراءة موثوقة تسمح باحتساب المستحق؛ ارفع Excel أو PDF نصيًا واضحًا.",tone:pdfDue?(pdfDue.dueTotal?"warn" as const:"ok" as const):"bad" as const}:undefined;
  const aging=single&&(supplier||customer)&&closingUsable?buildPdfAging(single,asOf,supplier,creditDays):undefined,supplierFlow=single&&supplier&&closingUsable?buildPdfSupplierFlow(single):undefined,agingMatches=!!aging&&aging.total===closingValue;
  ...
  const findings:AnalysisResult["findings"]=[
    ...
    ...(supplier&&single?[{title:"قاعدة المستحق المثبتة",detail:`إجمالي رصيد المورد ${closingValue}، والمستحق حسب مدة ${creditDays} يومًا ${dueValue}. بعد فاتورة القطع خُصمت مردودات المشتريات وسندات الصرف فقط.`,tone:"ok" as const},{title:"تصنيف المشتريات والمبيعات",detail:"يفك النظام الحروف العربية المتقطعة أو المعكوسة أولًا لتحديد الفواتير والسداد والمرتجع وإشعار الخصم.",tone:"ok" as const}]:[{title:customer?"قاعدة مستحق العميل المثبتة":"ملخص الحركة المحاسبية",detail:single&&customer&&pdfDue?`إجمالي رصيد العميل ${closingValue}، والمستحق حسب مدة ${creditDays} يومًا ${dueValue}. بعد فاتورة القطع خُصمت مردودات المبيعات وسندات القبض فقط.`:movementSummary,tone:single&&closingUsable?"ok" as const:"info" as const}]),
  ...
  const dueTable=single&&(supplier||customer)&&closingUsable?{headers:["رقم الفاتورة/المرجع","تاريخ إصدار الفاتورة","تاريخ الاستحقاق","أساس الاستحقاق","العمر بالأيام","فئة العمر","مدة التأخير/المتبقي","الحالة","الرصيد المفتوح"],rows:pdfDueRows(single,asOf,creditDays).slice(-150)}:undefined;
  return{title:supplier?"تحليل مشتريات ومبيعات المورد من PDF":pdfTitle(action),confidence:closingUsable||(!supplier&&!customer&&quality>=.55)?"متوسطة":"محدودة",conclusion,dueScenarios:pdfDue?.dueScenarios,dueSchedule:pdfDue?.dueSchedule,aging,supplierFlow,dueTable,summary:[{label:"عدد الصفحات",value:String(pages)},{label:"الحركات المحاسبية",value:String(movements.length)},{label:"إجمالي الرصيد حسب الكشف",value:closingValue},{label:`غير مستحق خلال ${creditDays} يومًا`,value:pdfDue?money(pdfDue.nonDueTotal):"غير مؤكد",tone:"ok"},{label:dueLabel,value:single?(supplier||customer?dueValue:money(single.closing)):"راجع رصيد كل ملف",tone:single&&closingUsable?(pdfDue?.dueTotal?"warn":"ok"):"bad"}],findings,table:{headers:supplier?supplierHeaders:customer?customerHeaders:generalHeaders,rows:tableRows},checks:[`تمت قراءة ${pages} صفحة و${totalLines} سطرًا نصيًا.`,`قاعدة ثابتة: تاريخ الاستحقاق = تاريخ إصدار الفاتورة + ${creditDays} يومًا.`,`بعد آخر فاتورة مستحقة يُخصم فقط ${supplier?"مردودات المشتريات وسندات الصرف والسداد":"مردودات المبيعات وسندات القبض والتحصيل"} اللاحقة، ولا تُضاف الفواتير الأحدث غير المستحقة.`,accountingRule,closingConfirmed?"تمت مطابقة صافي النتيجة مع مصدر الرصيد الموضح في التقرير.":runningBalanceUsable?"تم عرض آخر رصيد جارٍ كقيمة مبدئية تحتاج مراجعة قبل الاعتماد.":"لم يتم اعتماد مبلغ مستحق غير مؤكد.",movements.length>300?"يظهر أحدث 300 حركة في الجدول حتى تكون آخر العمليات ظاهرة، ويمكن تنزيل النتائج بصيغة CSV.":"تم عرض كل الحركات المؤكدة في الجدول.","لا يتم إنشاء أو ترحيل أي قيد محاسبي تلقائيًا."],sources:ds.map(d=>d.fileName)};
}
...
function prepareLedgerAging(d:DataSet,supplier:boolean,closing:number,now:Date,creditDays=30){
  ...
  const rows=lots.map(lot=>{const invoiceAge=Math.max(0,Math.floor((now.getTime()-lot.date.getTime())/86400000)),timing=dueTiming(lot.date,now,creditDays),range=agingRanges(creditDays).find(item=>invoiceAge>=item.min&&invoiceAge<=item.max);return[lot.ref||"—",pdfDateFormatter.format(lot.date),pdfDateFormatter.format(timing.dueDate),`تاريخ الفاتورة + ${creditDays} يومًا`,String(invoiceAge),range?.label||`${invoiceAge} يومًا`,timing.timing,timing.status,money(lot.amount)]});
  ...
  const dueTotal=dueAt(creditDays),nonDueTotal=Math.max(0,Math.abs(closing)-dueTotal),scenarioDays=[...new Set([30,45,60,creditDays])].filter(days=>days>=30&&days<=60).sort((a,b)=>a-b);
  ...
  const lastIncluded=dueLots.at(-1),nextDueInvoice=ledgerRows.filter(row=>row.isInvoice&&!isLotDue({date:row.date,amount:Math.max(0,row.change),ref:row.ref},now,creditDays)).sort((a,b)=>effectiveDueDate({date:a.date,amount:0},creditDays).getTime()-effectiveDueDate({date:b.date,amount:0},creditDays).getTime()||a.index-b.index)[0],nextDueCutoff=nextDueInvoice?ledgerRows.filter(row=>row.isInvoice&&row.date.getTime()===nextDueInvoice.date.getTime()).at(-1):undefined,nextDueDate=nextDueInvoice?effectiveDueDate({date:nextDueInvoice.date,amount:0},creditDays):undefined,nextDueDeductions=nextDueCutoff?ledgerRows.filter(row=>row.index>nextDueCutoff.index&&row.isDeduction).reduce((sum,row)=>sum-row.change,0):0,nextDueBalance=nextDueCutoff?Math.max(0,nextDueCutoff.balance-nextDueDeductions):0;
  const nextDueInvoicesAmount=nextDueInvoice?ledgerRows.filter(row=>row.isInvoice&&row.date.getTime()===nextDueInvoice.date.getTime()).reduce((sum,row)=>sum+Math.max(0,row.change),0):0;
  const dueSchedule={asOf:pdfDateFormatter.format(now),includedThrough:selectedCalculation.cutoff?pdfDateFormatter.format(effectiveDueDate({date:selectedCalculation.cutoff.date,amount:0},creditDays)):lastIncluded?pdfDateFormatter.format(effectiveDueDate(lastIncluded,creditDays)):"لا توجد فاتورة مستحقة",...(selectedCalculation.cutoff?{cutoffInvoiceDate:pdfDateFormatter.format(selectedCalculation.cutoff.date),cutoffBalance:money(selectedCalculation.cutoff.balance),laterDeductions:money(selectedCalculation.deductions)}:{}),...(nextDueInvoice&&nextDueDate?{nextDueDate:pdfDateFormatter.format(nextDueDate),nextDueAmount:money(nextDueBalance),nextDueInvoicesAmount:money(nextDueInvoicesAmount),nextDueRef:nextDueInvoice.ref||"—",nextDueDays:remainingDays(now,nextDueDate),nextDueCreditDays:creditDays}:{})};
...
function ledger(ds: DataSet[], supplier=false,creditDays=30): AnalysisResult {
  ...
  const unpaidInvoiceList=!bal&&!!debit&&!credit&&transactionRows.length>0&&transactionRows.every(row=>/فاتوره.*(?:مبيعات|بيع)|sales invoice/.test(accountingSearchText(Object.values(row).join(" ")))),lastBalanceRow=bal?[...d.rows].reverse().find(row=>String(value(row,bal)??"").trim()!==""):undefined,closing=bal&&lastBalanceRow?number(value(lastBalanceRow,bal)):supplier?totalCredit-totalDebit:totalDebit-totalCredit,lastDate=[...d.rows].reverse().map(r=>parseDate(value(r,ageDate))).find((x):x is Date=>!!x),reportDate=parseDate(d.reportAsOf),asOf=reportDate?new Date(reportDate):unpaidInvoiceList?new Date():lastDate?new Date(lastDate):new Date();asOf.setHours(0,0,0,0);
  const prepared=prepareLedgerAging(d,supplier,closing,asOf,creditDays),supplierSheet=supplier?prepareSupplierSheet(d,closing):undefined,customerSheet=!supplier?prepareCustomerSheet(d,closing):undefined,missing=(unpaidInvoiceList?[debit,ageDate]:[debit,credit,ageDate]).filter(x=>!x).length,dueLabel=supplier?"الرصيد الفعلي المستحق للمورد":"الرصيد الفعلي المستحق على العميل",dueValue=prepared.dueTotal;
  ...
  const findings:AnalysisResult["findings"]=[
    ...
    {title:"مصدر صافي الرصيد",detail:bal?"تم أخذ الصافي من آخر خلية رصيد غير فارغة في الكشف.":unpaidInvoiceList?"تم التعرف على الملف كقائمة فواتير مفتوحة غير مسددة؛ إجمالي الرصيد هو مجموع قيمة الفواتير، دون إعادة خصم سندات قبض غير موجودة في هذا التقرير.":"لم يظهر عمود رصيد؛ تم احتساب الصافي من المدين والدائن.",tone:bal||unpaidInvoiceList?"ok":"warn"},
  ...
  const dueTable=prepared.rows.length?{headers:["رقم الفاتورة/المرجع","تاريخ إصدار الفاتورة","تاريخ الاستحقاق","أساس الاستحقاق","العمر بالأيام","فئة العمر","مدة التأخير/المتبقي","الحالة","الرصيد المفتوح"],rows:prepared.rows.slice(-150)}:undefined;
...
export function analyzeData(action: string, ds: DataSet[],creditDays=30): AnalysisResult {
  ...
  if(ds.every(d=>d.kind!=="pdf")){const first=ds[0],ledgerRequest=request.includes("مورد")||request.includes("عميل")||request.includes("ديون")||request.includes("استحقاق"),inventoryRequest=request.includes("جرد")||request.includes("مخزون")||request.includes("صنف"),journalRequest=request.includes("قيود اليوميه");
    if(ledgerRequest&&(!column(first,"date")&&!column(first,"invoiceDate")||(!column(first,"balance")&&!(column(first,"debit")&&column(first,"credit"))&&!(column(first,"debit")||column(first,"amount")))))throw new Error("توقّف التحليل: لم يتم التعرف بثقة على تاريخ الفاتورة وقيمتها أو أعمدة الرصيد. لن يعرض البرنامج نتيجة قبل قراءة الأعمدة بصورة صحيحة.");
