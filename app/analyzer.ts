...
function bank(ds: DataSet[]): AnalysisResult {
  ...
  const systemOnly=systemRows.filter((_,index)=>!used.has(index)),amountDifferences:{bank:BankMovement;system:BankMovement}[]=[];for(const bankMovement of [...bankOnly]){const bankRef=referenceKey(bankMovement.ref);if(!bankRef)continue;const candidate=systemOnly.find(systemMovement=>{const systemRef=referenceKey(systemMovement.ref),dateDiff=bankMovement.date&&systemMovement.date?Math.abs(bankMovement.date.getTime()-systemMovement.date.getTime())/86400000:0;return !!systemRef&&(bankRef.includes(systemRef)||systemRef.includes(bankRef))&&dateDiff<=3&&Math.abs(bankMovement.amount-systemMovement.amount)>.02});if(candidate)amountDifferences.push({bank:bankMovement,system:candidate})}
  const differingBank=new Set(amountDifferences.map(x=>x.bank.row)),differingSystem=new Set(amountDifferences.map(x=>x.system.row)),pureBankOnly=bankOnly.filter(x=>!differingBank.has(x.row)),pureSystemOnly=systemOnly.filter(x=>!differingSystem.has(x.row));
